...
if (debug_mode) {
    ...
    print(paste0("Loading the data file: ", data_files))
    ...
    print(paste0("Using SNP db file: ", db_file))
}
...
get_data <- function(id, data = NULL) {
    ...
    if (!gtools::invalid(data)) {
        if (is.matrix(ds$data)) {
            stop(sprintf("only 1 data element in '%", id, "'"))
        }
...
get_dataset_info <- function() {
    ...
    for (d in datasets) {
        ...
        if (tolower(ds$datatype) == "mrna") {
            annotations <- 
                list(ids = ds$annot.mrna$gene.id)
...
get_lod_scan <- function(dataset, id, intcovar = NULL, cores = 0) {
    ...
    # creating a temporary tibble so we can do an inner_join and make sure
    # the marker_ids match up
    temp_tib <- as_tibble(temp)
    temp_tib$marker.id <- dimnames(temp)[[1]]
    
    ret <- 
        inner_join(temp_tib, markers, c("marker.id"="marker.id")) %>% 
...
get_snp_assoc_mapping <- function(dataset, id, intcovar, chrom, location,
    ...
    if (nvl_int(window_size, -1) == -1) {
        stop(paste0("window_size is invalid: ", tmp))
    }
